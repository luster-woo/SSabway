...
/**
 * 역무원 화면이 점으로 찍을 사용자의 역 내 현재 위치 노드.
 *
 * 명세 예시값("S3_02")을 그대로 쓴다 — 도면 그래프(daeguNavigation.ts 의
 * DAEGU_NODES)에 실존하는 노드라 지도에 정상적으로 찍힌다.
 *
 * ⚠️ 도면 그래프가 갱신되면 이 노드가 남아 있는지 확인할 것. 없는 노드면
 *    UserLocationModal 이 "위치 정보가 없습니다"로 안내한다.
 */
export const MOCK_USER_LOCATION_NODE = 'S3_02'
