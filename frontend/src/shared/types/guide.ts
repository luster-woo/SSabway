 ...
 *
 * 두 값 모두 사람이 읽는 한 줄 표기다. 출발은 표지판 노드를 지도 데이터로 옮긴
 * 위치 문구이고(describeStationPoint), 도착은 탄 노선의 개찰구 표기다. 매핑에
 * 없는 노드 코드만 예전처럼 "역 이름 + 코드" 로 남는다.
 */
export interface GuideInfo {
  /** 출발 — 표지판으로 인식한 지점. 예: "대구역 3층 6번 출구 앞" */
  origin: string
  /** 도착 — 그 역에서 향할 지점(개찰구 등). 예: "대구역 1호선 개찰구" */
  destination: string
}
