...
import { ToastContext, type ToastOptions } from '@/shared/ui/toastContext'
...
export function ToastProvider({ children }: { children: ReactNode }) {
  ...
  const showToast = useCallback((next: string, options?: ToastOptions) => {
    setToast({ message: next, plain: options?.plain ?? false })
      ...
      >
        {toast ? (
          <p
            className={
              toast.plain
                ? 'text-ink max-w-[360px] text-center text-[16px] leading-relaxed font-bold'
                : 'max-w-[360px] rounded-full bg-[#252c36] px-5 py-3 text-center text-[13.5px] text-white shadow-lg'
            }
          >
            {toast.message}
          </p>
